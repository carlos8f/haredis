...
tests.EVAL_1 = function() {
  ...
  client.set( "error reply key", "error reply value", function( err, res ) {
    ...
    client.eval( "local foo = redis.pcall('incr','error reply key'); return {type(foo),foo['err']}", 0,
      function( err, res ) {
        if( err ) {
          throw err;
        }
...
tests.UNSUB_EMPTY_CB = function() {
  ...
  if( !server_version_at_least( client, [2, 6, 11] ) ) {
    return next( name );
  }
...
tests.MGET = function() {
  var name = "MGET";
  client.mset( ["mget keys 1", "mget val 1", "mget keys 2", "mget val 2", "mget keys 3", "mget val 3"],
    require_string( "OK", name ) );
...
client.once( "ready", function start_tests() {
  console.log( "Connected to " + client.host + ":" + client.port + ", Redis server version " +
